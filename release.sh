...
#   NOTARY_PROFILE="daisy-notary" sh release.sh                 # package only
#   NOTARY_PROFILE="daisy-notary" GITEA_TOKEN=xxxx sh release.sh # package + upload
#   NOTARY_PROFILE="daisy-notary" sh release.sh v1.2             # explicit version tag
#
...
build_dmg() { # $1 = arch
    ...
    cp "$MENU_ICON_PNG" "$app/Contents/Resources/daisy.png"
    if [ -d "$bin_dir/DaisyTranslator_DaisyTranslator.bundle" ]; then
        ...
        cp "$MENU_ICON_PNG" "$app/Contents/Resources/DaisyTranslator_DaisyTranslator.bundle/daisy.png"
...
for dmg in "$ARTIFACT_DIR/daisy-$VERSION-arm64.dmg" "$ARTIFACT_DIR/daisy-$VERSION-x86_64.dmg"; do
    name=$(basename "$dmg")
