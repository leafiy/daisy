...
body="Native macOS translation helper for menu bar and clipboard workflows.\n\nRecommended install:\n\n    curl -fsSL $GITEA_URL/$OWNER_REPO/raw/branch/main/install.sh | sh\n\nManual install: download daisy-$VERSION-arm64.dmg (Apple Silicon) or daisy-$VERSION-x86_64.dmg (Intel), verify it with SHA256SUMS, then drag Daisy into Applications."
release_json=$(curl -sf -X POST -H "$AUTH" -H "Content-Type: application/json" \
    -d "{\"tag_name\":\"$VERSION\",\"name\":\"Daisy $VERSION\",\"body\":\"$body\",\"target_commitish\":\"$HEAD_SHA\"}" \
...
for asset in "$ARTIFACT_DIR/daisy-$VERSION-arm64.dmg" "$ARTIFACT_DIR/daisy-$VERSION-x86_64.dmg" "$ARTIFACT_DIR/SHA256SUMS"; do
    name=$(basename "$asset")
